...
function buildData() {
  return Array.from({ length: 30 }, (_, folderIndex) => ({
    id: `folder-${folderIndex + 1}`,
    label: `文件夹 ${folderIndex + 1}`,
    children: Array.from({ length: 12 }, (_, fileIndex) => ({
      id: `folder-${folderIndex + 1}-file-${fileIndex + 1}`,
      label: `文件 ${folderIndex + 1}-${fileIndex + 1}.txt`,
    })),
  }))
}

const data = ref<TreeNode[]>(buildData())
</script>

<template>
  <FaTree :data="data" node-key="id" default-expand-all virtual-list />
</template>
```
