...
开启后端生成后，路由权限有两种做法，一种是后端直接返回用户具备访问权限的路由数据（不推荐），另一种则返回全部的路由数据，让框架自行处理。

两种做法的区别在于第一种返回的路由数据里，无需在 meta 对象里设置 auth 参数，并且由于无权限的路由不会返回给前端，所以访问时会直接提示 404 ，而第二种对于无权限访问的路由，框架会以 403 页面进行展示。
